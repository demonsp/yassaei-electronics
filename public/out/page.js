...
function sectionCards(sections) {
  ...
  return sections.map((s) => html`
    <div class="card mt">
      ...
      ${bullets(list4(s, "list").length ? list4(s, "list") : s.list || [])}
    </div>`).join("");
}
      ...
      ${(page.items || []).map((it) => html`
        <div class="pwa-card">
          ...
          <div>
            ...
            ${bullets(list4(it, "list").length ? list4(it, "list") : it.list || [])}
          </div>
        </div>`)}
    </div>
    ${sectionCards(page.sections)}`;
...
var init_page = __esm({
  "public/js/views/page.mjs"() {
    ...
    list4 = (o, k) => isFa2() ? o?.[k] || [] : o?.[`${k}En`] || o?.[k] || [];
