...
var SECTIONS, GROUPS, GROUPS_EN, allowedSections, findEntry, title13;
var init_admin = __esm({
  "public/js/views/admin/index.mjs"() {
    ...
    allowedSections = () => SECTIONS.filter((s) => (!s.perm || can(s.perm)) && (!s.feat || feat(s.feat)));
...
async function render33(ctx) {
  const sec = ctx.params.section || "";
  const nav = SECTIONS2.filter((s) => !s.feat || feat(s.feat));
  const body = await sectionHtml2(sec, ctx);
  return html`
    <div class="section-head"><div>
...
async function sectionHtml2(sec, ctx) {
  switch (sec) {
    case "orders":
      return ordersHtml();
    case "wishlist":
...
var SECTIONS2, addrHandle2, ticketHandle, fbHandle, title14;
