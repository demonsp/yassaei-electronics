...
var L, STATUS_CLASS, PRI_CLASS, F4, THREADS;
var init_tickets = __esm({
  "public/js/views/admin/tickets.mjs"() {
    ...
    STATUS_CLASS = { open: "bp-warn", answered: "bp-success", closed: "bp-muted" };
    ...
    act("adm-sup-send", async (e, form) => {
      ...
      try {
        ...
        if (box) {
          ...
          wrap.innerHTML = messageHtml({ from: "staff", staffName: S.me?.name || "", body, at: (/* @__PURE__ */ new Date()).toISOString() }).trim();
...
async function mount28(root, ctx) {
  applyDyn(root);
  const sec = ctx.params.section || "";
  const entry = findEntry(sec);
...
function renderFooter() {
  ...
  qs("#fContact").innerHTML = html`
    ...
    <li>${icon("clock")}<span>${t("footer.workingHours")}: ${(st.workingHours || []).map((w) => `${isFa2() ? w.fa : w.en} ${isFa2() ? w.time : w.timeEn}`).join(" \xB7 ")}</span></li>`;
...
var init_actions = __esm({
  "public/js/actions.mjs"() {
    ...
    act("accept-consent-now", async (e, el2) => {
      ...
      sessionStorage.removeItem("consentDeferred");
      const { maybeConsent: maybeConsent3 } = await Promise.resolve().then(() => (init_main(), main_exports));
      maybeConsent3();
      Promise.resolve().then(() => (init_router(), router_exports)).then((m) => m.refresh());
    });
  }
  ...
  return html`
    ...
    <div class="card">
      <div class="row row-between row-wrap">
        ...
        <div class="row row-wrap">
          <span class="badge-pill ${STATUS_CLASS2[ticket.status] || "bp-muted"}">${t(`tk.${ticket.status}`)}</span>
          <span class="badge-pill ${PRI_CLASS2[ticket.priority] || "bp-info"}">${t(`tp.${ticket.priority}`)}</span>
    ...
    <div class="card mt">
      ...
      <div class="tk-thread mt-s" data-tk-thread>
        ${(ticket.messages || []).map(messageHtml2).join("")}
      </div>
...
var STATUS_CLASS2, PRI_CLASS2, pendingFiles;
var init_ticket_detail = __esm({
  "public/js/views/ticket-detail.mjs"() {
    ...
    init_actions();
    init_router();
    STATUS_CLASS2 = { open: "bp-warn", answered: "bp-success", closed: "bp-muted", in_progress: "bp-info", new: "bp-info" };
    ...
    act("tk-reply", async (e, form) => {
      ...
      await withBusy(btn, async () => {
        try {
          ...
          if (thread) {
            thread.innerHTML = (r.ticket?.messages || []).map(messageHtml2).join("");
